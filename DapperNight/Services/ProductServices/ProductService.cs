...
namespace DapperNight.Services.ProductServices
{
    public class ProductService : IProductService
    {
        ...
        {
            var query = "insert into TblProduct (ProductName,Stock,Price,CategoryId) values (@productName,@stock,@price,@categoryId)";
        ...
        {
            var query = "Select ProductId,ProductName,CategoryName,Stock,Price From TblProduct inner join TblCategory on TblProduct.CategoryId=TblCategory.CategoryId";
            var connection = _context.CreateConnection();
        ...
        {
            var query = "Select ProductId, ProductName,TblProduct.CategoryId,CategoryName,Stock,Price From TblProduct inner join TblCategory on TblProduct.CategoryId=TblCategory.CategoryId Where ProductId=@productId";
            var parameters = new DynamicParameters();
            parameters.Add("@productId", id);
            var connection = _context.CreateConnection();
            var value = await connection.QueryFirstAsync<GetByIdProductDto>(query,parameters);
            return value;
        }

        public async Task UpdateProductAsync(UpdateProductDto updateProductDto)
        {
            var query = "Update TblProduct Set ProductName=@productName,Stock=@stock,Price=@price,CategoryId=@categoryId Where ProductId=@productId";
            var parameters = new DynamicParameters();
